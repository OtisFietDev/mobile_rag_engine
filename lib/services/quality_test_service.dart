...
class QualityTestService {
  ...
  static final List<TestCase> testCases = [
    TestCase(
      query: "fruit",
      relevantDocs: ["Apple", "Banana", "Orange", "Grape", "Watermelon", "Strawberry"],
      category: "Fruits",
    ),
    TestCase(
      query: "red fruit",
    ...
    TestCase(
      query: "tech company",
      relevantDocs: ["Tesla", "Apple", "Google", "Samsung", "Microsoft", "Amazon"],
      category: "Tech",
    ),
    TestCase(
      query: "phone company",
  ...
  /// Calculate Recall@K
  static double _calculateRecall(List<String> actual, List<String> expected, int k) {
    int hits = 0;
  ...
  /// Calculate Precision
  static double _calculatePrecision(List<String> actual, List<String> expected, int k) {
    int hits = 0;
    final topK = actual.take(k).toList();
