...
class BenchmarkService {
  ...
  static Future<BenchmarkResult> benchmarkTokenize(String text, {int iterations = 50}) async {
    return benchmark(
  ...
  static Future<BenchmarkResult> benchmarkEmbed(String text, {int iterations = 10}) async {
    return benchmark(
  ...
  static Future<List<BenchmarkResult>> runFullBenchmark({
    ...
    final mediumText = "Apples are red fruits rich in vitamins. Eating them daily is good for health.";
    final longText = "Apples belong to the rose family and are one of the most widely cultivated fruits in the world. "
    ...
    final batchTexts = [
      ...
      "Dogs are cute and loyal", "Cats are agile hunters",
      "Cars are fast vehicles", "Computers are convenient tools",
      "Paris is the capital of France", "The ocean is vast and blue", "Mountains are tall and majestic",
    ];
    
    // Sequential embedding (for comparison)
    results.add(await benchmark(
    ...
    final testDbPath = "${(await getApplicationDocumentsDirectory()).path}/benchmark_db.sqlite";
    await initDb(dbPath: testDbPath);
    
    // Sample documents (20 texts x 5 = 100 documents)
    final sampleTexts = [
      "Apple is delicious", "Banana is yellow", "Orange is round", "Grape is sweet", "Watermelon is big",
      "Dog is cute", "Cat is agile", "Rabbit is fast", "Turtle is slow", "Monkey is smart",
      "Car is fast", "Bicycle is healthy", "Airplane flies in the sky", "Ship crosses the ocean", "Train arrives on time",
      "Computer is convenient", "Smartphone is essential", "Tablet is light", "Laptop is portable", "Desktop is powerful",
    ];
    ...
    for (var i = 0; i < 5; i++) {
      for (final text in sampleTexts) {
        ...
        await addDocument(dbPath: testDbPath, content: "$text ($i)", embedding: emb);
      }
    }
